...
@Config
@Autonomous(name = "modredauto", group = "Autonomous")
public class modredauto extends DbzOpMode {
    ...
    public static double gate1x = 127.5;
    public static double gate1h = 25;
...
//Shooting after 2nd time opening gate
            Path11 = follower.pathBuilder().addPath(
                    new BezierCurve(
                            new Pose(96.274, 82.893),
                            new Pose(98.598, 84.288),
                            new Pose(xend-4.5, 83.648)
                    )
            ).setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(0)).build();
    ...
    public void opLoop() {
        ...
        switch (state) {
            ...
            case 17:
                if (!follower.isBusy()) {
                    beginWait(500, true, 25); //if we don't go for back balls
                }
                break;
